...
from rnp_numpy import any
from rnp_numpy import array, asarray, concatenate, errstate, ndarray
from .numerictypes import complex128, flexible, float64, int_
from .printoptions import format_options
from rnp_numpy import absolute, isfinite, isinf


# ---------------------------------------------------------------------------
# rnp adaptation layer
#
# Everything below stands in for a numpy internal the port does not (yet)
# expose in the same shape.  The body of this module is otherwise upstream's
# `numpy/_core/arrayprint.py` verbatim.
# ---------------------------------------------------------------------------

from .. import _printing as _pr
...
def array_function_dispatch(dispatcher=None, module=None, **kwargs):
    def decorator(fn):
...
def _opt(v):
    """numpy spells "argument not given" as -1 inside the dragon4 wrappers."""
    return None if v is None or v < 0 else v


def dragon4_positional(x, precision=-1, min_digits=-1, unique=True,
                       fractional=True, trim='k', sign=False, pad_left=None,
                       pad_right=None):
    return _pr.format_float_positional(
        x, precision=_opt(precision), unique=unique, fractional=fractional,
        trim=trim, sign=sign, pad_left=_opt(pad_left),
        pad_right=_opt(pad_right), min_digits=_opt(min_digits))


def dragon4_scientific(x, precision=-1, min_digits=-1, unique=True, trim='k',
                       sign=False, pad_left=None, exp_digits=None):
    return _pr.format_float_scientific(
        x, precision=_opt(precision), unique=unique, trim=trim, sign=sign,
        pad_left=_opt(pad_left), exp_digits=_opt(exp_digits),
        min_digits=_opt(min_digits))
...
def _boolean_select(a, mask):
    """``a[mask]`` — but the port cannot boolean-index a 0-d array.

    numpy's answer for a 0-d array is the 1-d array of the elements the 0-d
    mask keeps, which is exactly what indexing the 1-element reshape gives.
    """
    if a.ndim == 0:
        return a.reshape(1)[mask.reshape(1)]
    return a[mask]
...
class FloatingFormat:
    ...
    def fillFormat(self, data):
        ...
        finite_vals = _boolean_select(data, isfinite(data))
        ...
        if data.size != finite_vals.size:
            neginf = self.sign != '-' or any(_boolean_select(data, isinf(data)) < 0)
...
class _TimelikeFormat:
    def __init__(self, data):
        non_nat = _boolean_select(data, ~isnat(data))
        if len(non_nat) > 0:
...
# ---------------------------------------------------------------------------
# The public dragon4 handles are the ones already validated against numpy in
# `rnp_numpy._printing`; the wrappers above exist only so the module reads
# like upstream's.
# ---------------------------------------------------------------------------

format_float_positional = _pr.format_float_positional
format_float_scientific = _pr.format_float_scientific
