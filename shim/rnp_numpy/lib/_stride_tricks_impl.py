# ---------------------------------------------------------------------------
# Ported from upstream numpy lib/_stride_tricks_impl.py (v2.5.2), verbatim except for the
...
#   L8: from numpy._core.numeric import normalize_axis_tuple
#      -> from ._rnp_compat import normalize_axis_tuple
#   L9: from numpy._core.overrides import array_function_dispatch, set_module
#      -> from ._rnp_compat import array_function_dispatch, set_module
#   L10: from numpy.lib._array_utils_impl import byte_bounds
#      -> from rnp_numpy.lib._array_utils_impl import byte_bounds
# ---------------------------------------------------------------------------
"""
...
#
# Upstream builds strided views by round-tripping an `__array_interface__`
# dict through `np.asarray`, and broadcasts with `np.nditer`.  The engine
# exposes both operations natively (`_rnp._as_strided`, `broadcast_to`,
# `broadcast_shapes`), so the three functions that need them are redefined
# here on top of the verbatim upstream body above.
# ---------------------------------------------------------------------------
...
def _broadcast_to(array, shape, subok, readonly):
    if not shape and np.ndim(array):
        raise ValueError('cannot broadcast a non-scalar to a scalar array')
    shape = tuple(shape) if np.iterable(shape) else (shape,)
    if any(size < 0 for size in shape):
        raise ValueError('all elements of broadcast shape must be non-'
                         'negative')
    return np.broadcast_to(np.asanyarray(array) if subok
                           else np.asarray(array), shape)


def _broadcast_shape(*args):
    return np.broadcast_shapes(*(np.shape(a) for a in args))
